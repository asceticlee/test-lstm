#!/usr/bin/env python3
"""
Paradigm-Specific LSTM Training for Stock Price Regression

This script trains LSTM models on data from a specific market paradigm.
It uses the paradigm classifications to filter training data to only include
weeks that belong to the specified paradigm, allowing for specialized models
that are optimized for specific market conditions.

Usage:
    python stockprice_lstm_paradigm_regression.py <paradigm_number> <labelNumber> [--validation_split 0.2]
    
Example:
    python stockprice_lstm_paradigm_regression.py 0 10 --validation_split 0.2
"""
...
def load_paradigm_data(paradigm_number):
    """Load and filter data for a specific paradigm"""
    print(f"Loading data for paradigm {paradigm_number}...")
    ...
    paradigm_data = paradigm_df[paradigm_df['Paradigm'] == paradigm_number]
    
    if len(paradigm_data) == 0:
        raise ValueError(f"No data found for paradigm {paradigm_number}")
    
    print(f"Found {len(paradigm_data):,} data points for paradigm {paradigm_number}")
    
    # Get unique weeks for this paradigm
    paradigm_weeks = paradigm_data['Week'].unique()
    print(f"Paradigm {paradigm_number} spans {len(paradigm_weeks)} weeks")
    ...
    filtered_df = full_df[full_df['TradingDay'].isin(paradigm_trading_days)].reset_index(drop=True)
    
    print(f"Filtered to {len(filtered_df):,} rows for paradigm {paradigm_number}")
    
    return filtered_df, paradigm_weeks

def split_paradigm_data(df, paradigm_weeks, validation_split=0.2, random_seed=42):
    ...
    shuffled_weeks = np.random.permutation(paradigm_weeks)
    
    n_val_weeks = int(len(paradigm_weeks) * validation_split)
    ...
    day_to_week = dict(zip(paradigm_df['TradingDay'], paradigm_df['Week']))
...
def train_paradigm_model(paradigm_number, label_number, validation_split=0.2, 
                        seq_length=15, learning_rate=0.0009, epochs=150, batch_size=32):
    """Train LSTM model for a specific paradigm"""
    print(f"\n{'='*60}")
    print(f"Training LSTM model for Paradigm {paradigm_number}, Label {label_number}")
    print(f"{'='*60}")
    
    # Load paradigm data
    df, paradigm_weeks = load_paradigm_data(paradigm_number)
    
    # Split into train/validation by weeks
    train_df, val_df = split_paradigm_data(df, paradigm_weeks, validation_split)
    ...
    # Generate model ID
    model_log_path = models_dir / 'paradigm_model_log.csv'
                    ...
                    if isinstance(model_id_field, str) and '_' in model_id_field:
                        try:
                            ...
                            if len(parts) == 2:
                                ...
                                if paradigm_part == paradigm_number:
    ...
    model_id_str = f"P{paradigm_number:02d}_{model_id:05d}"
    ...
    # Save model
    model_path = models_dir / f'lstm_paradigm_model_{model_id_str}.keras'
    ...
    base_row = [model_id_str, paradigm_number, label_number, validation_split, seq_length,
                learning_rate, epochs, batch_size, len(paradigm_weeks) - int(len(paradigm_weeks) * validation_split),
                int(len(paradigm_weeks) * validation_split), len(y_train), len(y_val),
    ...
    print(f"Paradigm: {paradigm_number}")
...
def main():
    parser = argparse.ArgumentParser(description='Train paradigm-specific LSTM models')
    parser.add_argument('paradigm_number', type=int, help='Paradigm number to train on (0-7, based on your paradigm analysis)')
    ...
    # Validate paradigm number (check against actual paradigm analysis)
    paradigm_file = paradigm_dir / 'paradigm_assignments.csv'
    if paradigm_file.exists():
        paradigm_df = pd.read_csv(paradigm_file)
        available_paradigms = sorted(paradigm_df['Paradigm'].unique())
        max_paradigm = max(available_paradigms)
        min_paradigm = min(available_paradigms)
        
        if args.paradigm_number < min_paradigm or args.paradigm_number > max_paradigm:
            raise ValueError(f"Paradigm number must be between {min_paradigm} and {max_paradigm}. Available paradigms: {available_paradigms}")
    else:
        raise FileNotFoundError(f"Paradigm assignments file not found: {paradigm_file}")
    ...
    # Check if paradigm analysis exists
    if not paradigm_dir.exists():
        raise FileNotFoundError(f"Paradigm analysis directory not found: {paradigm_dir}")
    
    # Train model
    model_id, val_mae = train_paradigm_model(
        paradigm_number=args.paradigm_number,
