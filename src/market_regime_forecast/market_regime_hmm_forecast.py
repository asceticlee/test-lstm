...
class HMMRegimeForecaster:
    ...
    def extract_daily_features(self, market_data):
        ...
        if self.mode == 'daily':
            # Mode 1: Use SAME data period as GMM clustering (10:35 AM to 12:00 PM)
            # to ensure feature consistency with regime labels
            filtered_data = market_data[
                (market_data['ms_of_day'] >= self.intraday_cutoff_ms) &  # 10:35 AM start
                (market_data['ms_of_day'] <= self.intraday_end_ms)       # 12:00 PM end
            ].copy()
            reference_time_ms = self.intraday_cutoff_ms  # Use 10:35 as reference (same as GMM)
            print(f"Daily mode: Using SAME period as GMM clustering: {self.intraday_cutoff_time} to {self.intraday_end_time}")
            print(f"This ensures feature consistency with regime labels generated by GMM")
