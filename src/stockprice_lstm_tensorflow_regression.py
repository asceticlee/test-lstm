...
print(f'Train sequences shape: {X_train_bal.shape}, Train labels shape: {y_train_bal.shape}')
print(f'Test sequences shape: {X_test.shape}, Test labels shape: {y_test.shape}')

# Scale features (fit on train, apply to both)
scaler = MinMaxScaler()
X_train_reshaped = X_train_bal.reshape(-1, num_features)
X_test_reshaped = X_test.reshape(-1, num_features)

scaler.fit(X_train_reshaped)
X_train_scaled = scaler.transform(X_train_reshaped).reshape(X_train_bal.shape)
...
history = model.fit(X_train_scaled, y_train_bal, epochs=epochs, batch_size=batch_size,
...
train_results_df = pd.DataFrame({
    'Actual': y_train_bal,
    'Predicted': train_predictions.flatten()  # Flatten to match y_train_bal's shape
})
...
print(f"Saved all {len(y_train_bal)} train predictions and actuals to '{train_output_file}'")
