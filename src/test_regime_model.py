#!/usr/bin/env python3
"""
Test Regime-Specific LSTM Models

This script tests regime-specific LSTM models on data from specific date ranges.
It can test models on their own regime data or cross-regime data to evaluate
specialization effectiveness.

Usage:
    python test_regime_model.py <model_id> <test_from> <test_to> [--accuracy-only]
    
Example:
    python test_regime_model.py P00_00001 20250701 20250707 --accuracy-only
"""
...
def test_regime_model(model_id, test_from, test_to, accuracy_only=False):
    """
    Test a regime model on a specific date range and return results.
    
    Args:
        model_id: Regime model ID string (e.g., "P00_00001")
    ...
    try:
        ...
        model_path = model_dir / f'lstm_regime_model_{model_id}.keras'
        ...
        # Get model parameters from regime model log
        paradigm_log_path = model_dir / 'regime_model_log.csv'
            ...
            with open(paradigm_log_path, 'r') as f:
                ...
                for row in reader:
                    if row.get('model_id') == model_id:
                        ...
                        paradigm_number = int(row.get('regime', 0))
                        break
        ...
        # Get regime information for test data
        ...
        if paradigm_file.exists():
            ...
            if len(test_paradigm_data) > 0:
                test_paradigms = test_paradigm_data['Regime'].value_counts()
...
def main():
    parser = argparse.ArgumentParser(description='Test regime-specific LSTM models')
    parser.add_argument('model_id', type=str, help='Regime Model ID (e.g., P00_00001)')
    ...
    try:
        if args.accuracy_only:
            result = test_regime_model(args.model_id, args.testFrom, args.testTo, accuracy_only=True)
            print(','.join(result))
        else:
            result = test_regime_model(args.model_id, args.testFrom, args.testTo, accuracy_only=False)
            
            if "error" in result:
                print(f"ERROR: {result['error']}", file=sys.stderr)
                sys.exit(1)
            
            print(f"Testing Regime Model: {result['model_id']}")
            print(f"Trained on Regime: {result['paradigm_number']}")
