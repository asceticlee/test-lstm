#!/usr/bin/env python3
"""
Market Regime Classification using Gradient Boosting Machine
...
class MarketParadigmClassifier:
    ...
    def classify_paradigms(self, n_paradigms=None):
        ...
        self.weekly_features['Regime'] = paradigm_labels
    ...
    def analyze_paradigms(self):
        ...
        for regime in sorted(self.weekly_features['Regime'].unique()):
            paradigm_data = self.weekly_features[self.weekly_features['Regime'] == regime]
    ...
    def train_gbm_classifier(self, X_pca, paradigm_labels):
        """Train GBM classifier for regime prediction on new data"""
        print("Training GBM classifier for regime prediction...")
        ...
        plt.title('Top 20 PCA Components for Regime Classification')
    ...
    def predict_paradigm(self, week_features):
        ...
        # Predict regime
        regime = self.gbm_classifier.predict(X_pca)
    ...
    def visualize_paradigms(self):
        ...
        colors = plt.cm.Set3(np.linspace(0, 1, len(paradigm_timeline['Regime'].unique())))
        
        for i, regime in enumerate(sorted(paradigm_timeline['Regime'].unique())):
            paradigm_data = paradigm_timeline[paradigm_timeline['Regime'] == regime]
        ...
        plt.subplot(2, 2, 3)
        paradigm_counts = paradigm_timeline['Regime'].value_counts().sort_index()
        plt.pie(paradigm_counts.values, labels=[f'Regime {i}' for i in paradigm_counts.index], 
                autopct='%1.1f%%', colors=colors[:len(paradigm_counts)])
        plt.title('Regime Distribution')
        
        # Plot 4: Weekly return distribution by regime
        plt.subplot(2, 2, 4)
        for regime in sorted(paradigm_timeline['Regime'].unique()):
            paradigm_data = paradigm_timeline[paradigm_timeline['Regime'] == regime]
            plt.hist(paradigm_data['Week_Return'], alpha=0.5, label=f'Regime {regime}', bins=20)
        
        plt.title('Weekly Return Distribution by Regime')
        ...
        if sns is not None:
            for regime in sorted(self.weekly_features['Regime'].unique()):
                ...
                if len(available_features) > 1:
                    ...
                    plt.tight_layout()
                    plt.savefig(self.output_dir / f'paradigm_{regime}_correlation.png', 
