...
target_col = 'Label_8'  # Raw continuous label for binary categorization
num_features = len(feature_cols)

# Function to create sequences, respecting continuity, and force binary labels (up if >0, down if <=0; no drop)
def create_sequences(df, seq_length):
    ...
    X = []
    y = []  # Binary labels: 0=down (<=0), 1=up (>0)
    ...
    while i <= len(df) - seq_length:
        ...
        if is_consecutive:
            ...
            # Force binary: up if >0, down if <=0 (no threshold, no skip)
...
# Save model
model.save('lstm_binary_full_data_model.keras')
...
train_output_file = os.path.join(project_root, 'train_predictions_binary_full.csv')
...
output_file = os.path.join(project_root, 'test_predictions_binary_full.csv')
